...
def training(Xtrain, Ytrain, batch_size, epochs, neurals):
    ...
    model = Sequential()
    model.add(GRU(units=neurals, activation='sigmoid',
...
def graph(time, series, XtrainLen, trainPredict, XvalidLen, validPredict, testPredict):
    ...
    ax.set_title('GRU MODEL')
    ax.legend(loc='upper right')
    ax.grid(True)
    ax.set_xlabel('Time (day)', fontsize=16)
    ax.set_ylabel('Gas Rate' + r'$(m^3)$', fontsize=16)
    ...
    for i in range(len(series.values)):
        actualCumlative.append(sum(series['GAS'].values[0:i]))

    prediction = pd.DataFrame(np.concatenate((trainPredict, validPredict, testPredict), axis=0), columns=['GAS'])
    predictedCumulative = list()
    for i in range(len(prediction)):
        predictedCumulative.append(sum(prediction['GAS'].values[0:i]))
    ...
    ax.set_title('CUMULATIVE PRODUCTION GAS RATE')
    ax.legend(loc='upper left')
    ax.grid(True)
...
def assemble(series, steps, train_size, batch_size, neurals, epochs, time):
    ...
    gru = training(Xtrain, Ytrain, batch_size, epochs, neurals)

    train_prediction = list()
    for i in range(len(Xtrain)):
        Yhat = predict(gru, 1, Xtrain[i])
    ...
    output = pd.DataFrame(np.concatenate((time.values[0:len(time) - 1], prediction), axis=1), columns=['STEP', 'GAS'])
    ...
    gruModel = gru.to_json()
    with open('lstmModel.json', 'w') as json_file:
        json_file.write(gruModel)
    gru.save_weights("gruWeight.h5")
...
def run():
    ...
    series = rawData[['GAS']]
    time = rawData[['STEP']]
    assemble(series, steps=5, train_size=0.7, batch_size=1, neurals=1, epochs=10, time=time)
