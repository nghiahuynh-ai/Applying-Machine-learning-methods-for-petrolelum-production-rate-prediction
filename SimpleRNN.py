...
def training(Xtrain, Ytrain, batch_size, epochs, neurals):
    ...
    model = Sequential()
    model.add(SimpleRNN(units=neurals, activation='sigmoid',
...
def assemble(series, steps, train_size, batch_size, neurals, epochs, time):
    ...
    rnn = training(Xtrain, Ytrain, batch_size, epochs, neurals)

    train_prediction = list()
    for i in range(len(Xtrain)):
        Yhat = predict(rnn, 1, Xtrain[i])
    ...
    output.to_csv('Volve_well/outputRNN.csv')
    output.to_excel('Volve_well/outputRNN.xlsx')

    rnnModel = rnn.to_json()
    with open('lstmModel.json', 'w') as json_file:
        json_file.write(rnnModel)
    rnn.save_weights("rnnWeight.h5")
