...
def saveOutput(learner, labelName, actual, predicted, cumulative):
    ...
    output.to_csv(labelName + 'rateLR.csv')
    output.to_excel(labelName + 'rateLR.xlsx')

    filename = labelName + 'LR.sav'
...
def plotLinearData(labelName, step, label):
    plt.rc('font', size=12)
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(step, label)
    ax.set_title('LINEARIZED ' + labelName + ' RATE')
    ax.set_xlabel('TIME (DAY)')
    ax.set_ylabel('TRANSFORMED ' + labelName + ' RATE')
    ax.grid(True)
...
def plotActualVsPred(labelName, ytrain, train_pred, yvalid, valid_pred, ytest, test_pred):
    ...
    ax.set_title('ACTUAL VS PREDICTED ' + labelName + ' RATE VALUES - LINEAR REGRESSION')
...
def plotRateVsTime(labelName, step, label, trainStep, train_pred, validStep, valid_pred, testStep, test_pred):
    ...
    ax.set_title(labelName + ' RATE PREDICTION BASED ON LINEAR REGRESSION')
    ax.set_xlabel('TIME (DAY)')
...
def plotCumulative(labelName, step, realCumulative, predCumulative):
    ...
    ax.set_title('CUMULATIVE ' + labelName + ' RATE - LINEAR REGRESSION')
    ax.set_xlabel('TIME (DAY)')
    ax.set_ylabel(labelName + ' RATE (CUBIC METER)')
    ax.grid(True)
    ax.legend()
...
def main():
    ...
    plotLinearData(labelName, feature['STEP'], label ** (1 / exp))
